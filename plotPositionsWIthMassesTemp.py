...
#for i in range(tstep,tstep+1):
	...
	title = 'Halo Positions with Masses inputtemp'
	fig = plt.figure()
	...
	if len(positionX[mask])>0:
		ax.scatter(positionX[mask], positionY[mask], positionZ[mask],
			   c='#848484',s=math.log(nodeData.nodeMass[i])/10,edgecolors='none')
	...
	if len(positionX[mask])>0:
		ax.scatter(positionX[mask], positionY[mask], positionZ[mask],
			   c='#424242',s=math.log(nodeData.nodeMass[i]),edgecolors='none')
	for i in range(nHalos):
		mask[i] = False
	...
	if len(positionX[mask])>0:
		   ax.scatter(positionX[mask], positionY[mask], positionZ[mask],
			      c='#DF0174',s=math.log(nodeData.nodeMass[i])*5,edgecolors='none')
	for i in range(nHalos):
		mask[i] = False
	...
	if len(positionX[mask])>0:
		ax.scatter(positionX[mask], positionY[mask], positionZ[mask],
			   c='#9E0050',s=math.log(nodeData.nodeMass[i])*25,edgecolors='none')
	for i in range(nHalos):
		mask[i] = False
		if(1E14<nodeMass[i]):
			mask[i] = True & pmask[i]
	if len(positionX[mask])>0:
		ax.scatter(positionX[mask], positionY[mask], positionZ[mask],
			   c='#610B4B',s=math.log(nodeData.nodeMass[i])*50,edgecolors='none')		   
        # Set the axis labels
